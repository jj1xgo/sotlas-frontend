...
### Phase 5 事前相談の結果（2026-07-14、Fable。計画立案は次回セッションで実施）

PR #45〜47 マージ後の実測に基づく Phase 5 のスコープ確定材料:

- 残存は `$parent.close()` 6箇所・3ファイルのみ（EditAlert/EditSpot/EditPhoto、Buefy モーダル子の
  慣用句。Buefy 3 での契約確認が必要）。`$root`/`$on`/`filters:`/`$set(`/`slot-scope`/`.sync` は全て 0 件。
  eslint は vue3-essential 適用・警告ゼロ維持済み。bulma 整合は Phase 2 で解消済み
- **追加タスク（ユーザー合意済み）**: Phase 5 完了報告として upstream issue #44 へ人間向けの移行総括
  コメントを書く。内容は「何が変わったか（自前モジュール一覧・src/mapgl/README.md の場所）／
  何が変わっていないか（Options API・構造維持）／既知の残課題」。メンテナの懸念
  「移行後にコードベースを理解できる人間がいなくなる」への直接の応答。リポジトリ内への
  ドキュメント追加はこのコメントで要否をメンテナに尋ねる（勝手に足さない）
- **計画時に要判断（未決）**: (a) README.md の陳腐化修正（`npm run test` は実在しない・vue-cli 時代の
  リンク。移行以前からの問題で必須ではない、小PR候補）を Phase 5 に含めるか、
  (b) テスト基盤の新規提案（移行スコープ外・メンテナ合意が要る別件）を #44 で提案するか
- 上記以外の着手前懸念（依存更新・fork パッチ・bulma・upstream との進め方）は解消済みと確認。
  Phase 6 は不要との結論。`vue3`→`master` マージと本番デプロイはメンテナ側の判断・作業
