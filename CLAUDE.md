...
- **ローカルパッチ**（上流には出さない master 固有の差分）: コンテナ/セッション運用整備一式
  （`.claude/`・`.claude-container.d/`・`.gitignore`・`.mcp.json`・本ファイル自身）に加え、
  `src/store.js`・`src/App.vue` のコンテナ内 Turnstile 検証バイパス（issue #18。開発ビルド限定、詳細は
...
- **コンテナ内での WebUI 動作確認**: Playwright MCP（リポジトリルート `.mcp.json`、コンテナ内 headless
  ...
  - **Turnstile 制約**: Cloudflare Turnstile（`challenges.cloudflare.com`）がヘッドレスブラウザの
    bot 検知で通過できないため、地図が絡む画面（`/map`、summit 詳細の埋め込みミニマップ）は
    issue #18 のバイパス（`import.meta.env.DEV` 限定、`VITE_MAPTILER_DEV_KEY`）が前提。この変数は
    `.claude-container.d/env` の独自変数（claude-container 組み込みの `_FILE` 解決機構の対象外、
    詳細は issue #20）で、直接値をコンテナへ渡す汎用手段が無いため、`npm run dev` 起動時に
    セッション限定で export する（`permissions.deny` が `Bash(env:*)` を禁止しているため
    `env` コマンドでなく shell 組み込みを使う）:
    ```
    export VITE_MAPTILER_DEV_KEY="$(grep '^VITE_MAPTILER_DEV_KEY=' .claude-container.d/env | cut -d= -f2-)"
    npm run dev
    ```
  - ホスト側で `claude` を起動した場合、`/usr/bin/chromium`（コンテナ専用に導入）が無いため MCP の
    ブラウザ起動はエラーになる
- **ホスト側での動作確認**: 上記で確認できない場合（ヘッドフル確認・Turnstile 不通過時の地図確認等）の
  補助手段として、ホスト側で `npm run dev` を実行し実ブラウザで確認する（ホスト側の `.env` には
  `VITE_MAPTILER_DEV_KEY` を追加しないため、本物の Turnstile フローのまま検証できる）
...
- コンテナはポートを公開しない。ブラウザ確認は上記「検証方法」を参照（コンテナ内 Playwright MCP を
  第一手段、ホスト側は補助手段）
